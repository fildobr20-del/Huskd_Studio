...
const TOKEN_RATES: Record<string, number> = { chaturbate: 0.05, stripchat: 0.05, bongacams: 0.021 }
...
async function fetchChaturbate(username: string): Promise<PlatformEarnings> {
  try {
    ...
    if (!res.ok) return { platform: "chaturbate", tokens: 0, amount: 0 }
    const data = await res.json()
    const cashout = data.stats?.find((s: any) => s.program === "Cashed-Out Tokens")
    if (!cashout) return { platform: "chaturbate", tokens: 0, amount: 0 }
    const totalTokens = cashout.totals?.Tokens || 0
    const totalPayout = cashout.totals?.Payout || 0
    return { platform: "chaturbate", tokens: totalTokens, amount: totalPayout > 0 ? Number(totalPayout) : Number(totalTokens) * TOKEN_RATES.chaturbate }
  } catch { return { platform: "chaturbate", tokens: 0, amount: 0 } }
}
...
export async function GET(request: Request) {
  try {
    ...
    if (profile.is_demo) {
      return NextResponse.json({
        ...
        platformBreakdown: [
          ...
          { name: "SkyPrivate", amount: 294, tokens: 0 },
          { name: "Flirt4Free", amount: 168, tokens: 0 },
        ],
      })
    }
    ...
    // === 1. FETCH LIVE API (cumulative values) ===
    const liveData: Record<string, PlatformEarnings> = {}
    if (nicks.chaturbate) liveData.chaturbate = await fetchChaturbate(nicks.chaturbate)
    ...
    const today = now.toISOString().split("T")[0]
    // Sum saved earnings per platform for current month
    const savedMonthByPlatform: Record<string, number> = {}
    // Sum saved earnings per platform all time
    ...
    // === 3. DELTA AUTO-SAVE ===
    // API returns cumulative month total. We already saved some of it on previous days.
    // Delta = API_cumulative - already_saved_this_month
    // Only save if delta > $0.50 (avoid tiny floating point noise)
    if (!ghostId) {
      for (const [platform, live] of Object.entries(liveData)) {
        if (live.amount <= 0) continue
        const alreadySavedThisMonth = savedMonthByPlatform[platform] || 0

        // For Chaturbate: API returns ALL-TIME total, not monthly
        // So compare with ALL-TIME saved, not just this month
        const isAllTime = platform === "chaturbate"
        const alreadySaved = isAllTime
          ? (savedAllByPlatform[platform] || 0)
          : alreadySavedThisMonth
        const delta = Math.round((live.amount - alreadySaved) * 100) / 100
        if (delta > 0.50) {
          // Save delta as today's earnings
          const { data: existing } = await sb
            ...
            .single()
          if (existing) {
            // Today already has an entry — ADD delta to it
            ...
            await sb.from("earnings_daily").insert({
              user_id: uid, date: today, amount: delta, platform
            })
          }
